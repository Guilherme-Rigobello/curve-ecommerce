...
const ProductCard = ({ id, name, brand, image, feminine, price }) => {
  return (
    <article className='flex flex-col justify-around border-2 border-gray-200 rounded-lg card-produto group w-48 bg-stone-100 shadow-xl shadow-slate-200 overflow-hidden'>
      ...
      <button className='flex justify-center bg-slate-950 border border-transparent rounded-sm mx-4 my-3 p-2 text-md text-slate-100 hover:bg-transparent hover:text-slate-950 hover:border hover:border-slate-950 transition duration-200'>
        <FaCartPlus/>
      </button>
    </article>
  );
